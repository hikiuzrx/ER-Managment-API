...
export class AuthService {
  ...
  async login(identifier:string,password:string){
     let user:User = await this.userService.retriveUserByEmail(identifier)
     if(!user){
          user = await this.userService.retriveUserByUsername(identifier)
          if(!user){
               throw new NotFoundException('there is no user with this identifier')
          }
     }
     const auth :boolean = await this.userService.comparePassword(user,password)
     if(!auth){
